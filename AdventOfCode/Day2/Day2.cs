...
{
    [Test]
    public void GameEvaluatorSumsIdsOfValidGames()
    {
        IGameInterpreter gameInterpreter = A.Fake<IGameInterpreter>();
        IGameAnalyser gameAnalyser = A.Fake<IGameAnalyser>();
        IGameCombiner gameCombiner = A.Fake<IGameCombiner>();

        GameEvaluator gameEvaluator = new GameEvaluator(gameInterpreter, gameAnalyser, gameCombiner);

        Bag bag = new Bag();

        A.CallTo(() => gameInterpreter.ReadGames(A<string>.Ignored)).Returns(new List<Game> { new Game() });
        A.CallTo(() => gameAnalyser.Analyse(A<Game>.Ignored, bag)).Returns(new AnalysedGame { Valid = true });
        A.CallTo(() => gameCombiner.CombineGames(A<IEnumerable<AnalysedGame>>.Ignored)).Returns(5);
    ...
    public void GameValidatorShouldAllowGamesWithDrawsThatDontExceedTheBallBag()
    {
        ...
        Assert.That(gameValidator.Analyse(game, bag).Valid, Is.EqualTo(true));
    }

    [Test]
    public void GameValidatorShouldDenyGamesWithADrawThatExceedsTheBallBag()
    {
        var gameValidator = new GameValidator();
        ...
        Assert.That(gameValidator.Analyse(game, bag).Valid, Is.EqualTo(false));
    }
    ...
    public void GameCombinerSumsIdsOfGames()
    {
        ...
        var result = gameCombiner.CombineGames(new List<AnalysedGame> {
            new AnalysedGame { Id = 1, Valid = true },
            new AnalysedGame { Id = 2, Valid = true }
        });
    ...
    public void Part2Example()
    {
        GameEvaluator evaluator = new GameEvaluator(
            new GameInterpreter(), 
            new GameBagMinimiser(), 
            new MinimumGameBagCombiner()
        );
        ...
        Assert.That(evaluator.Evaluate(ExampleData, exampleBag), Is.EqualTo(2286));
    }
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
    ...
    public string ExampleData2 = @"Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
